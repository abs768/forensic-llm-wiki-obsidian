...
| **Vector RAG** | Embedded chunks of `raw_sources/`, cosine top-k | Semantic lookups where the query and the evidence share no keywords | Same as Raw RAG: retrieves, never reconciles | *"Was anything set to start automatically?"* → surfaces the Run-key lines without the word "autostart" appearing anywhere |
...
passed **19 / 23**, Vector RAG passed **10 / 23**, Raw RAG passed
**7 / 23**, and GraphRAG-lite passed **5 / 23**. Vector RAG (real
`all-MiniLM-L6-v2` embeddings) shows what better retrieval buys — and
what it does not: it surfaces both sides of a contradiction without
reconciling them, and the refusal and narrative gaps persist.
GraphRAG-lite performed best on its intended niche
